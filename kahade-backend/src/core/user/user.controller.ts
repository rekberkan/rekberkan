...
export class UserController {
  ...
  async changePassword(
    ...
    if (dto.newPassword.length > 128) {
      throw new BadRequestException('New password must not exceed 128 characters');
    }
  ...
  async uploadKYC(
    ...
    if (signatures) {
      ...
      const isValid = signatures.some((sig) => fileHeader.subarray(0, sig.length).equals(sig));
      if (!isValid) {
    ...
    FileInterceptor('avatar', {
      ...
      fileFilter: (req, file, callback) => {
        if (!ALLOWED_IMAGE_TYPES.includes(file.mimetype)) {
          return callback(new BadRequestException('Avatar must be JPEG, PNG, or WebP'), false);
        }
        callback(null, true);
      },
    }),
