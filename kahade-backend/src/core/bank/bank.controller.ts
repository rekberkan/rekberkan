...
export class BankController {
  ...
  async getBankAccount(@CurrentUser('id') userId: string, @Param('id', ParseUUIDPipe) id: string) {
  ...
  async addBankAccount(@CurrentUser('id') userId: string, @Body() dto: AddBankAccountDto) {
    // Validate bank code
    ...
    if (existingAccounts.length >= MAX_BANK_ACCOUNTS) {
      throw new BadRequestException(`Maximum ${MAX_BANK_ACCOUNTS} bank accounts allowed`);
    }
  ...
  @ApiResponse({ status: 429, description: 'Too many requests' })
  async setAsDefault(@CurrentUser('id') userId: string, @Param('id', ParseUUIDPipe) id: string) {
    const account = await this.bankRepository.findById(id, userId);
    if (!account) {
      throw new NotFoundException('Bank account not found');
    }
