...
export class BruteForceService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.MAX_FAILED_ATTEMPTS = this.configService.get<number>('BRUTE_FORCE_MAX_ATTEMPTS', 5);
    this.LOCKOUT_DURATION_MS = this.configService.get<number>('BRUTE_FORCE_LOCKOUT_MS', 15 * 60 * 1000);
    this.ATTEMPT_WINDOW_MS = this.configService.get<number>('BRUTE_FORCE_WINDOW_MS', 5 * 60 * 1000);
    ...
    if (redisUrl) {
      try {
        ...
        this.redis.connect().catch((error) => {
          this.logger.warn(`Redis connection failed, using in-memory fallback: ${error.message}`);
          this.redis = null;
        });
      } catch (error) {
        this.logger.warn(`Redis initialization failed, using in-memory fallback: ${error.message}`);
        this.redis = null;
      }
    } else {
      this.logger.warn('REDIS_URL not configured, using in-memory brute force protection (not suitable for distributed deployments)');
    }
  }
  ...
  async checkAccountLock(identifier: string): Promise<LockStatus> {
    ...
    if (this.redis) {
      try {
        ...
        const lockedUntil = data.lockedUntil ? new Date(parseInt(data.lockedUntil, 10)) : null;
   ...
   */
  async recordFailedAttempt(identifier: string): Promise<{ locked: boolean; attemptCount: number }> {
    const key = `${this.KEY_PREFIX}${identifier}`;
    const now = Date.now();
    ...
    if (this.redis) {
      try {
        ...
        if (count >= this.MAX_FAILED_ATTEMPTS) {
          ...
          this.logger.warn(`Account ${identifier} locked due to ${count} failed attempts`);
        }

        // Set expiry on the key (cleanup after lockout period + buffer)
        pipeline.expire(key, Math.ceil((this.LOCKOUT_DURATION_MS + this.ATTEMPT_WINDOW_MS) / 1000));

        await pipeline.exec();

        return { locked: count >= this.MAX_FAILED_ATTEMPTS, attemptCount: count };
      } catch (error) {
    ...
    if (!attempts) {
      this.failedAttempts.set(identifier, { count: 1, lastAttempt: new Date() });
    ...
    if (attempts.count >= this.MAX_FAILED_ATTEMPTS) {
      ...
      this.logger.warn(`Account ${identifier} locked due to ${attempts.count} failed attempts`);
    }

    this.failedAttempts.set(identifier, attempts);
    return { locked: attempts.count >= this.MAX_FAILED_ATTEMPTS, attemptCount: attempts.count };
  }
